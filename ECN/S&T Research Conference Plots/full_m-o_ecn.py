...
axs[0, 2].plot(density_list, Distortion, label="SnO", color="darkred", zorder=1)
...
axs[1, 0].set_xlabel(r'$\mathrm{Density, g/cm}^3$', fontsize=14)
axs[1, 1].set_xlabel(r'$\mathrm{Density, g/cm}^3$', fontsize=14)
axs[1, 2].set_xlabel(r'$\mathrm{Density, g/cm}^3$', fontsize=14)

axs[0, 0].set_ylabel("ECN", fontsize=14, labelpad=10)
axs[1, 0].set_ylabel("ECN Time Variance", fontsize=14, labelpad=10)
axs[0, 1].set_ylabel(r'L_Average, $\AA$', fontsize=14, labelpad=10)
axs[1, 1].set_ylabel("L_Average Time Variance", fontsize=14, labelpad=10)
axs[0, 2].set_ylabel(r'Distortion, $\AA^2$', fontsize=14, labelpad=10)
axs[1, 2].set_ylabel("Distortion Time Variance", fontsize=14, labelpad=10)
...
legend = axs[0, 1].legend(handles, labels, prop={'size': 11}, loc='upper center', bbox_to_anchor=(0.5, 1.4),
                 ncol=3, fancybox=True, shadow=True, fontsize='large')
legend.set_zorder(10000)
plt.draw()
axs[1, 0].set_yticks(np.arange(0.05, 0.4, 0.1))
axs[0, 1].set_yticks(np.arange(1.95, 2.25, 0.07))
axs[1, 1].set_yticks(np.arange(0.0005, 0.004, 0.00075))

#Scientific Notation for Y-Axis:

formatter1 = ticker.ScalarFormatter(useMathText=True) #for 1, 1
formatter1.set_powerlimits((-3, 3))  # Forces scientific notation when needed
formatter2 = ticker.ScalarFormatter(useMathText=True) #for 1, 2
formatter2.set_powerlimits((-3, 3))  # Forces scientific notation when needed
axs[1, 1].yaxis.set_major_formatter(formatter1)
axs[1, 1].yaxis.get_offset_text().set_visible(False)
axs[1, 2].yaxis.set_major_formatter(formatter2)
axs[1, 2].yaxis.get_offset_text().set_visible(False)

axs[1, 1].annotate(r"$\times 10^{-4}$", xy=(0.06, 0.9), xycoords='axes fraction', fontsize=12)
axs[1, 2].annotate(r"$\times 10^{-4}$", xy=(0.06, 0.9), xycoords='axes fraction', fontsize=12)
