...
while x < 4:
    ...
    axs[i,j].set_xlim(60, 180)
    axs[i,j].set_ylim(0, 0.043)
    axs[i,j].grid(True, alpha=0.25)
    axs[i,j].set_ylim(0.000, 0.043)
    axs[i,j].set_yticks(np.arange(0, 0.043, 0.01))
    axs[i, j].set_xticks(np.arange(60, 180, 20))
    ...
    if j == 2:
        j = 0
        i += 1
axs[0,1].legend(bbox_to_anchor=(1.0,0.148))
axs[1,1].legend(bbox_to_anchor=(1.0,0.220))
axs[1,0].set_xlabel(r'Angle ($^{\circ}$)', fontsize = '20')
axs[1,0].xaxis.set_label_coords(1.0,-.15)
axs[0,0].set_ylabel('Probability Density', fontsize = '20')
axs[0,0].yaxis.set_label_coords(-.25,0)
