...
file_names = ["../Data/Ta2SnO6_distrib_angle_MOM_density.dat", "../Data/Ta2SnO6_distrib_angle_OSnO_density.dat", "../Data/Ta2SnO6_distrib_angle_OTaO_density.dat"]
count = 0
...
with open("../Data/Ta2SnO6_density.dat") as g:
...
for a in file_names:
    ...
    colors_1 =['#610202', '#871e0d', '#ad390f', '#d2560e', '#f67503' , '#fba652', '#f9b86e', '#f8c88c']
...
while j < 3:
    ...
    axs[j].set_xlim(50, 180)
    axs[j].grid(True, alpha=0.25)
    axs[j].set_ylim(-.0001,0.038)
    axs[j].set_yticks(np.arange(0, 0.04, 0.01))

    axs[j].set_aspect(1.0 / axs[j].get_data_ratio(), adjustable='box')
    j += 1

axs[2].legend(loc = 'right', bbox_to_anchor=(2.0,0.5))
axs[0].set_xticks(np.arange(60, 180, 30))
axs[1].set_xticks(np.arange(60, 180, 30))
axs[2].set_xticks(np.arange(60, 180, 30))

axs[1].set_xlabel(r'Angle ($^{\circ}$)', fontsize = '20')
axs[1].xaxis.set_label_coords(.5,-.3)
...
fig.suptitle("Ta2SnO6", fontsize=25)
plt.show()